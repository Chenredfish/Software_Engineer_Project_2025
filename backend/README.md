...
**admin_crud_test.js** - CRUD 操作測試工具
```bash
# 測試會員表格的 CRUD 操作
node admin_crud_test.js member

# 測試電影表格的 CRUD 操作
node admin_crud_test.js movie

# 測試影城表格的 CRUD 操作
node admin_crud_test.js cinema

# 批量操作測試
node admin_crud_test.js batch

# 顯示使用說明
node admin_crud_test.js help
```
...
#### 👨‍💼 管理員專用資料查看 (完整實作)
- `GET /api/admin/dashboard` - 系統概覽 (所有表格統計)
- `GET /api/admin/members` - 查看所有會員資料 (含密碼)
- `GET /api/admin/movies` - 查看所有電影資料
- `GET /api/admin/cinemas` - 查看所有影城資料
- `GET /api/admin/showings` - 查看所有場次資料
- `GET /api/admin/bookings` - 查看所有訂票記錄
- `GET /api/admin/meals` - 查看所有餐點資料
- `GET /api/admin/ticketclasses` - 查看所有票種資料
- `GET /api/admin/supervisors` - 查看所有管理員資料 (含密碼)
- `GET /api/admin/orderstatus` - 查看所有訂單狀態資料
- `GET /api/admin/versions` - 查看所有電影版本資料
- `GET /api/admin/rated` - 查看所有電影分級資料
- `GET /api/admin/theaters` - 查看所有影廳資料
...
#### 👨‍💼 管理員 CRUD 操作 (完整實作)
- `POST /api/admin/create/:table` - 新增資料到任意表格 🔒
- `PUT /api/admin/update/:table` - 修改任意表格資料 🔒
- `DELETE /api/admin/delete/:table` - 刪除任意表格資料 🔒
- `POST /api/admin/batch` - 批量執行多個操作 🔒
...
**最新更新 (2025/12/16):**
- ✅ 管理員 CRUD 系統完全實作 (create/update/delete)
- ✅ 批量操作 API 完成
- ✅ 管理員測試工具完成 (admin_debug.js, admin_crud_test.js)
- ✅ 簡化輸出格式，提升使用體驗
...
└── routes/               # API 路由模組
    ...
    ├── admin.js          # 管理員 API (含完整CRUD)
    ├── admin_debug.js    # 管理員查詢工具
    └── admin_crud_test.js # CRUD 測試工具
```
